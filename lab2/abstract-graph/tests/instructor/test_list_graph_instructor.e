...
feature {NONE} -- Initialization
	...
	make
		...
		do
			...
			add_boolean_case (agent t22c)
			add_boolean_case (agent t23)
			add_boolean_case (agent t23b)
			add_boolean_case (agent t23c)
...
feature -- Add Vertex & Edge Command Tests
	...
	t7: BOOLEAN -- add_vertex, add_edge - Basic
		local
			i_g: LIST_GRAPH[INTEGER]
			i_v: VERTEX[INTEGER]
			i_e: EDGE[INTEGER]
		do
			comment ("t7: Add & Remove Commands - add_vertex, add_edge - Basic Integer Case")
			create i_g.make_empty
	...
	t8c: BOOLEAN -- add/remove vertices and edges - Self-Loop Course
		local
			c_g: LIST_GRAPH[COURSE]
			c_e: EDGE[COURSE]
			c1, c2, c3, c4, c5, c6: COURSE
...
feature -- Remove Vertex & Edge Command Tests (Integer)
	...
	t16b: BOOLEAN -- Remove last edge in graph String
		local
			...
			s_a: ARRAY[TUPLE[STRING, STRING]]
		do
			comment ("t16b: Add & Remove Commands - remove_edge - Remove Self Loop Edge String")
	...
	t16c: BOOLEAN -- Remove last edge in graph Course
		local
			...
			c1, c2, c3, c4, c5, c6: COURSE
		do
			comment ("t16c: Add & Remove Commands - remove_edge - Remove Self Loop Edge")
			create c_g.make_empty
			create c1.make (2011, "Data Structures", "F")
			create c2.make (2030, "Advanced Object Oriented Programming", "F")
			create c3.make (2031, "Software Tools", "W")
...
feature -- Basic Query Tests (Integer)
	...
	t22: BOOLEAN -- has_vertex Same Item, different Object
		local
			i_g: LIST_GRAPH[INTEGER]
			i_a: ARRAY[TUPLE[INTEGER, INTEGER]]
			i_v: VERTEX[INTEGER]
		do
			comment ("t22: Other Queries - has_vertex - Shares item, but is new object")
			i_a := << [1, 2], [1, 3], [1, 1], [3, 4], [3, 5], [5, 6], [6, 5] >>
			i_a.compare_objects
			create i_g.make_from_array (i_a)
			assert_equal ("correct vertices & edges", "[1:1,2,3][2][3:4,5][4][5:6][6:5]", i_g.out)
	...
	t23: BOOLEAN -- has_edge Basic Integer
		local
			i_g: LIST_GRAPH[INTEGER]
			i_a: ARRAY[TUPLE[INTEGER, INTEGER]]
		do
			comment ("t23: Other Queries - has_edge - Basic Integer")
			i_a := << [1, 2], [1, 3], [1, 1], [3, 4], [3, 5], [5, 6], [6, 5] >>
			i_a.compare_objects
			create i_g.make_from_array (i_a)
			assert_equal ("correct vertices & edges", "[1:1,2,3][2][3:4,5][4][5:6][6:5]", i_g.out)
...
feature -- Advanced Query Tests (Integer)
		...
		t27: BOOLEAN -- topologically_sorted Disconnected?
			local
				i_g: LIST_GRAPH[INTEGER]
				i_a: ARRAY[TUPLE[INTEGER, INTEGER]]
			do
				comment ("t27: Advanced Queries - topologically_sorted - Disconnected Graph")
				i_a := << [1, 2], [1, 3], [3, 4], [5, 6] >>
				i_a.compare_objects
				create i_g.make_from_array (i_a)
		...
		t28: BOOLEAN -- topologically_sorted Large Graph
			local
				i_g: LIST_GRAPH[INTEGER]
				i_a: ARRAY[TUPLE[INTEGER, INTEGER]]
			do
				comment ("t28: Advanced Queries - topologically_sorted - Large Graph")
				i_a := << [1, 2], [1, 4], [1, 5], [1, 8], [1, 3], [2, 3], [2, 6], [2, 4], [3, 4], [3, 7], [6, 7], [6, 8], [4, 8], [7, 8], [7, 4] >>
				i_a.compare_objects
				create i_g.make_from_array (i_a)
				assert_equal ("correct vertices & edges", "[1:2,3,4,5,8][2:3,4,6][4:8][5][8][3:4,7][6:7,8][7:4,8]", i_g.out)
				Result := i_g.edge_count ~ 15 and i_g.vertex_count ~ 8
				check Result end
		...
		t33: BOOLEAN -- Reachable Self Loop
			local
				i_g: LIST_GRAPH[INTEGER]
				i_a: ARRAY[TUPLE[INTEGER, INTEGER]]
				i_v: VERTEX[INTEGER]
				reachable: ARRAY[VERTEX[INTEGER]]
			do
				comment ("t33: Advanced Queries - reachable - Self Loop")
				i_a := << [1, 2], [1, 3], [3, 4], [5, 6], [6, 6] >>
				i_a.compare_objects
				create i_g.make_from_array (i_a)
				create i_v.make (10)
				i_g.add_vertex (i_v)
				assert_equal ("correct vertices & edges", "[1:2,3][2][3:4][4][5:6][6:6][10]", i_g.out)
				Result := i_g.edge_count ~ 5 and i_g.vertex_count ~ 7
				check Result end
		...
		t34: BOOLEAN -- is_topologically_sorted test
			local
				i_g: LIST_GRAPH[INTEGER]
				i_a: ARRAY[TUPLE[INTEGER, INTEGER]]
				v, seq: ARRAY[VERTEX[INTEGER]]
				i_v: VERTEX[INTEGER]

			do
				comment ("t34: Advanced Queries - is_topologically_sorted - Basic")
				i_a := << [1, 2], [2, 3], [2, 6], [2, 8], [4, 2], [4, 5], [5, 6], [7, 5], [7, 8] >>
				i_a.compare_objects
				create i_g.make_from_array (i_a)
				assert_equal ("correct vertices & edges", "[1:2][2:3,6,8][3][6][8][4:2,5][5:6][7:5,8]", i_g.out)
				Result := i_g.edge_count ~ 9 and i_g.vertex_count ~ 8
				check Result end

				-- Check different topological sortings
				create v.make_empty
				across
					1 |..| i_g.vertex_count as i
				loop
					create i_v.make (i.item)
					v.force (i_v, i.item)
				end

				seq := << v[1], v[4], v[7], v[2], v[5], v[3], v[6], v[8] >>
				Result := i_g.is_topologically_sorted (seq)
				check Result end

				seq := << v[7], v[1], v[4], v[5], v[2], v[3], v[6], v[8] >>
				Result := i_g.is_topologically_sorted (seq)
				check Result end

				seq := << v[1], v[4], v[7], v[5], v[2], v[8], v[6], v[3] >>
				Result := i_g.is_topologically_sorted (seq)
				check Result end
...
feature -- Object Equal Add/Remove/Has
	...
	t35:BOOLEAN -- add/remove - Remove vertex that shares same item as one in graph, but is a fresh vertex
		local
			i_g: LIST_GRAPH[INTEGER]
			i_v, i_v1: VERTEX[INTEGER]
			i_e: EDGE[INTEGER]
		do
			comment ("t35: Add & Remove Commands - remove_vertex - Vertex shares same item, but not reference equal")
			create i_g.make_empty
			across 1 |..| 6 as i loop
				create i_v.make (i.item)
				i_g.add_vertex (i_v)
			...
			assert_equal ("correct vertices & edges", "[1:2,5][2][3][4:3][5][6]", i_g.out)
			check i_g.edge_count ~ 3 and i_g.vertex_count ~ 6 end
	...
	t36:BOOLEAN -- Add/remove - Add Edge that doesn't exist in graph, but with source vertex that share the same item but are different than those added into the graph
		local
			i_g: LIST_GRAPH[INTEGER]
			i_v, i_v1: VERTEX[INTEGER]
			i_e: EDGE[INTEGER]
		do
			comment ("t36:  Add & Remove Commands - add_edge - source vertex not in graph (but shares same item)")
			create i_g.make_empty
			across 1 |..| 6 as i loop
				create i_v.make (i.item)
				i_g.add_vertex (i_v)
	...
	t37:BOOLEAN -- Add/remove - Add Edge that doesn't exist in graph, but with destination vertex that shares the same item but are different than those added into the graph
		local
			i_g: LIST_GRAPH[INTEGER]
			i_v, i_v2: VERTEX[INTEGER]
			i_e: EDGE[INTEGER]
		do
			comment ("t37: Add & Remove Commands - add_edge - destination vertex not in graph (but shares same item)")
			create i_g.make_empty
			across 1 |..| 6 as i loop
				create i_v.make (i.item)
				i_g.add_vertex (i_v)
	...
	t39:BOOLEAN -- Remove Edge with vertices not in the graph
		local
			i_g: LIST_GRAPH[INTEGER]
			i_a: ARRAY[TUPLE[INTEGER, INTEGER]]
			i_v1, i_v2: VERTEX[INTEGER]
			i_e: EDGE[INTEGER]
		do
			comment ("t39: Add & Remove Commands - remove_edge - Vertices share items with existing vertices, but are new objects")
			i_a := << [1, 2], [1, 3], [3, 4], [3, 5], [5, 6], [6, 5] >>
			i_a.compare_objects
			create i_g.make_from_array (i_a)
			create i_v1.make (10)
			...
			create i_e.make (i_v1, i_v2)
			i_g.remove_edge (i_e)
			assert_equal ("correct vertices & edges", "[1:3][2][3:4,5][4][5:6][6:5][10]", i_g.out) -- if this passes, it means that it actually removed the edge
			Result := i_g.edge_count ~ 5 and i_g.vertex_count ~ 7
		end
