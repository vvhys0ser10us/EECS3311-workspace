...
feature -- Creation Procedure Tests
	...
	t4b: BOOLEAN -- Command Testing - add_edge, outgoing, incoming - String
		local
			v1, v2: VERTEX[STRING]
			e1: EDGE[STRING]
		do
			comment ("t4b: Edges - add_edge, outgoing, incoming - String Case")
	...
	t6: BOOLEAN -- Command Testing - incoming_edge_count - Integer
		local
			v1, v2: VERTEX[INTEGER]
			e1: EDGE[INTEGER]
		do
	...
	t6b: BOOLEAN -- Command Testing - incoming_edge_count - String
		local
			v1, v2: VERTEX[STRING]
			e1: EDGE[STRING]
		do
			comment ("t6b: Queries - incoming_edge_count, outgoing_edge_count - String Case")
	...
	t6c: BOOLEAN -- Command Testing - incoming_edge_count - Course
		local
			v1, v2: VERTEX[COURSE]
			e1: EDGE[COURSE]
		do
	...
	t8: BOOLEAN -- Query Testing - incoming & outgoing edge_count - Integer Self Loop
		local
			v1, v2: VERTEX[INTEGER]
			e1: EDGE[INTEGER]
		do
			comment ("t8: Queries - edge_count - Integer Self-Loop Case")

			create v1.make (4)
			create v2.make (-1)
			create e1.make (v1, v2)
			v1.add_edge (e1)
			Result := v1.edge_count ~ 1 and v1.outgoing.has (e1)
			check Result end

			Result := v2.edge_count ~ 0
			check Result end

			v2.add_edge (e1)
			Result := v2.edge_count ~ 1 and v2.incoming.has (e1)
			check Result end

			create v2.make (3)
			create e1.make (v2, v1)
			v1.add_edge (e1)
			Result := v1.edge_count ~ 2 and v1.incoming.has (e1)
			check Result end
	...
	t12: BOOLEAN -- has_outgoing_edge - Check Object Comparison Integer
		local
			v1, v2: VERTEX[INTEGER]
	...
	t16: BOOLEAN -- has_incoming_edge - Integer Self Loop
		local
			v1, v2: VERTEX[INTEGER]
			e1, e2: EDGE[INTEGER]
		do
			comment ("t16: Queries - has_incoming_edge - Integer Self Loop")
			create v1.make (-55)
			create v2.make (1)
			create e1.make (v1, v2)
	...
	t18: BOOLEAN -- outgoing_sorted - Integer Self Loop
		local
			v1, v2, v3, v4: VERTEX[INTEGER]
			e: EDGE[INTEGER]
		do
			comment ("t18: Queries - outgoing_sorted - Integer Self Loop")
			create v1.make (1)
			create v2.make (4)
			create v3.make (3)
			create v4.make (2)
			...
			Result := v1.outgoing_sorted.count ~ 4

			assert_equal("outgoing_sorted_correct", "1:1,2,3,4", v1.out)
		end
