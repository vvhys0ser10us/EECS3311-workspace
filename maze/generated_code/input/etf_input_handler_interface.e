...
feature -- parsing
	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[ETF_EVT_ARG]]) : ETF_COMMAND_INTERFACE
		...
		do
			...
			if cmd_name ~ "new_game" then 
				 if attached {ETF_ENUM_INT_ARG} args[1] as a_level and then (a_level.value = easy or else a_level.value = medium or else a_level.value = hard) then 
					 create {ETF_NEW_GAME} Result.make ("new_game", [a_level.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 

			elseif cmd_name ~ "move" then 
				 if attached {ETF_ENUM_INT_ARG} args[1] as a_direction and then (a_direction.value = N or else a_direction.value = E or else a_direction.value = S or else a_direction.value = W) then 
					 create {ETF_MOVE} Result.make ("move", [a_direction.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			...
			elseif cmd_name ~ "solve" then 
				 if TRUE then 
					 create {ETF_SOLVE} Result.make ("solve", [], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			else 
	...
	do
		...
		loop
			...
			if cmd_name ~ "new_game" then 
				if NOT( ( args.count = 1 ) AND THEN attached {ETF_ENUM_INT_ARG} args[1] as a_level and then (a_level.value = easy or else a_level.value = medium or else a_level.value = hard)) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
							"new_game(a_level: LEVEL = {easy, medium, hard})")
				end

			elseif cmd_name ~ "move" then 
				if NOT( ( args.count = 1 ) AND THEN attached {ETF_ENUM_INT_ARG} args[1] as a_direction and then (a_direction.value = N or else a_direction.value = E or else a_direction.value = S or else a_direction.value = W)) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
							"move(a_direction: DIRECTION = {N, E, S, W})")
				end
